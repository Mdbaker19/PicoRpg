...
class Display(object):
    ...
    def draw_bitmap_array(self, bitmap_data, x, y, w, h, invert=False, rotate=0):
        """Draw MONO_HMSB bitmap from provided bytearray and draw to screen.

        Args:
            bitmap_data (bytearray): Bitmap data.
            x (int): x-coord of image.
            y (int): y-coord of image.
            w (int): Width of image.
            h (int): Height of image.
        ...
        """
        array_size = w * h
        fb = FrameBuffer(bitmap_data, w, h, MONO_HMSB)

        if rotate == 0 and invert:
            fb2 = FrameBuffer(bytearray(array_size), w, h, MONO_HMSB)
            for y1 in range(h):
                for x1 in range(w):
                    fb2.pixel(x1, y1, fb.pixel(x1, y1) ^ 0x01)
            ...
            for y1 in range(h):
                for x1 in range(w):
                    if invert:
                        fb2.pixel(x1, y1, fb.pixel((w - 1) - x1, (h - 1) - y1) ^ 0x01)
                    else:
    ...
    def draw_bitmap_array_raw(self, bitmap_data, x, y, w, h, invert=False, rotate=0):
        """Draw raw bitmap from provided bytearray and draw to screen.

        Args:
            bitmap_data (bytearray): Raw bitmap data.
            x (int): x-coord of image.
            y (int): y-coord of image.
            w (int): Width of image.
            h (int): Height of image.
        ...
        if rotate == 0:
            buf[:] = bitmap_data[:buf_size]
        elif rotate == 90:
            for x1 in range(w - 1, -1, -1):
                for y1 in range(h):
                    index = (w * y1) + x1
                    buf[index] = bitmap_data[index]
        elif rotate == 180:
            for index in range(buf_size):
                buf[index] = bitmap_data[buf_size - 1 - index]
        elif rotate == 270:
            for x1 in range(1, w + 1):
                for y1 in range(h - 1, -1, -1):
                    index = (w * y1) + x1 - 1
